...
### [계획 실행: 완성형 코드 계획의 태스크 리뷰]
- 규칙: 계획 문서에 완성형 코드가 들어 있어도 태스크별 리뷰 게이트를 생략하지 않는다 — 계획 코드 자체의 결함은 전사(transcription)로는 못 잡는다
- 근거: 2026-07-06 v0.8.0 실행 중 계획 코드 자체 결함 2건(SessionStart 주입 상한 누락, Stop 훅 null 기준선 오검)을 태스크 리뷰가 발견
- 신뢰도: 낮음(1)
