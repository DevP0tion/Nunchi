...
Codex CLI는 Claude Code와 호환되는 plugin/hooks 체계를 채택했다 (hooks.json 형식·전 이벤트 지원, `additionalContext` 주입, Stop `decision: block`, plugin 번들 MCP 서버, `CLAUDE_PLUGIN_ROOT` 호환 환경 변수까지 동일). Codex 지원은 이 `codex-support` 브랜치 전용이며, 마켓플레이스에 **`nunchi-codex`** 항목으로 등록되어 있다 (소스가 이 브랜치로 고정됨).
...
- **Bun 요구사항 동일** — hooks와 MCP 서버가 `bun`으로 실행되므로 PATH에 있어야 한다.
...
Claude Code든 Codex든, CLAUDE.md·AGENTS.md·메모리 문서에 경험을 적고 다음 세션에 읽는 자기개선은 플러그인 없이도 이미 가능하다. nunchi는 그 대체재가 아니라, 그중 **작업 강도 판단**이라는 한 영역을 골라 기록·회수·반증을 시스템으로 만든 것이다. 저장 매체 선택에는 양쪽 다 실익이 있다 — 문서의 강점은 **사람 쪽 인터페이스**(열람, 편집, git, 팀 공유, 무의존성)이고, DB의 강점은 **기계 쪽 연산**(검색 회수, 동시성, 필드 단위 갱신)이다. nunchi가 DB를 택한 것은 회수 파이프라인(매 메시지 검색 주입)이 설계의 중심이기 때문이고, 문서가 유리한 축(아래 표의 "문서 유리" 행)은 `doc()` 내보내기와 항목에 내장된 근거·날짜 필드로 완화한다.

| 축 | 문서 기반 자기개선 (CLAUDE.md, 메모리 문서 등) | nunchi (sqlite 보정 DB) |
|---|---|---|
