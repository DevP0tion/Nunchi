---
name: migrate
description: nunchi 보정 DB 마이그레이션이 필요할 때 사용 - 플러그인 업데이트 후 보정 항목이 안 보일 때, 구버전 데이터(0.7.x calibration.md, 0.8.x calibration 테이블, 0.9~0.12.x memory 테이블)를 최신 스키마(memory + events 저널)로 이전할 때, memory.db가 잠겨 있거나 구버전 서버가 떠 있어 마이그레이션이 적용되지 않을 때, 사용자가 "마이그레이션", "migrate", "DB 이전", "스키마 업그레이드"를 언급할 때.
---
...
| 0.9~0.12.x `memory` 테이블 | observe CHECK·promoted_to·refs 컬럼 추가(테이블 재구축, id 보존) + 기존 행을 `events` 저널에 add 이벤트로 부트스트랩 |

최신 스키마: `memory(id, section, area, rule, evidence, confidence, keywords, promoted_to, refs, updated_at)`(파생) + `memory_fts`(FTS5 trigram, 파생) + `events`(append-only canonical 저널) + `meta`(파생 반영 스탬프). 파생 상태는 events replay로 재구축 가능하다.
...
4. **검증** — `nunchi_list` 결과를 1번 기록과 대조한다: 항목 수 일치, id 보존. 하나라도 어긋나면 사용자에게 보고하고 진행을 멈춘다.
